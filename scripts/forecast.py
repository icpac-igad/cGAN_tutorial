...
input_folder = "/network/group/aopp/predict/TIP021_MCRAECOOPER_IFS/IFS-regICPAC-meansd/2019/"
input_files = fcst_params["INPUT"]["file"]
start_hour = fcst_params["INPUT"]["start_hour"]
end_hour = fcst_params["INPUT"]["end_hour"]
output_folder = "/network/group/aopp/predict/TIP022_NATH_GFSAIMOD/cGAN/predictions/Jurre_Brishti/2019/GAN_forecasts_2019_offset_large"#fcst_params["OUTPUT"]["folder"]
...
# %%
#input_file
for day in np.arange('2019-05-22','2019-12-31',np.timedelta64(1,'D'),dtype='datetime64[D]'):
    # %%
...
#    start_times = [nc_in["time"][0]]
    ...
    d = datetime(int(str(day).split('-')[0]),int(str(day).split('-')[1]),int(str(day).split('-')[2])) #+ timedelta(hours=int(start_times[0]))
